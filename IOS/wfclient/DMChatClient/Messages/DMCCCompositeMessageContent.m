...
#import "DMCCCompositeMessageContent.h"
#import "Common.h"
#import "DMCCIMService.h"
#import "DMCCMessage.h"

@implementation DMCCCompositeMessageContent
- (DMCCMessagePayload *)encode {
    ...
    for (DMCCMessage *msg in self.messages) {
        ...
        DMCCMessagePayload *msgPayload = [msg.content encode];
        ...
        if ([msgPayload isKindOfClass:DMCCMediaMessagePayload.class]) {
            DMCCMediaMessagePayload *mediaPayload = (DMCCMediaMessagePayload *)msgPayload;
...
- (void)decode:(DMCCMessagePayload *)payload {
    ...
    NSMutableArray<DMCCMessage *> *messages = [[NSMutableArray alloc] init];
    if (!__error && dictionary && [dictionary[@"ms"] isKindOfClass:[NSArray class]]) {
        ...
        for (NSDictionary *msgDict in arrays) {
            DMCCMessage *msg = [[DMCCMessage alloc] init];
            msg.messageUid = [msgDict[@"uid"] longLongValue];
            
            msg.conversation = [[DMCCConversation alloc] init];
            ...
            msg.content = [[DMCCIMService sharedDMCIMService] messageContentFromPayload:payload];
