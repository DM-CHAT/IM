...
static NSMutableDictionary *DMCUrlImageDict;
static NSLock *DMCImageLock;

@implementation DMCCUtilities
...
+ (UIImage *)fileToThumbnail:(NSString*)path{
    if(path == nil){
        return nil;
    }
    UIImage *ui = [UIImage new];
    ui = [ui initWithContentsOfFile:path];
    return [DMCCUtilities generateThumbnail:ui withWidth:120 withHeight:120];
}
+ (UIImage *)generateThumbnail:(UIImage *)image
    ...
    CGSize targetSize = [DMCCUtilities imageScaleSize:image.size targetSize:CGSizeMake(targetWidth, targetHeight) thumbnailPoint:&thumbnailPoint];
...
+ (UIImage *)getUserImage:(NSString *)url {
    ...
    [DMCImageLock lockBeforeDate:[NSDate dateWithTimeIntervalSinceNow:10]];
    UIImage *image = [DMCUrlImageDict objectForKey:url];
    if (!image) {
        image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:url]]];
        if (DMCUrlImageDict.count > 50) {
            [DMCUrlImageDict removeAllObjects];
        }
        if (image) {
            [DMCUrlImageDict setObject:image forKey:url];
        }
    }
    [DMCImageLock unlock];
...
+ (void)generateNewGroupPortrait:(NSString *)groupId
    ...
    NSNumber *createTime = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"DMC_group_generate_portrait_time_%@_%d", groupId, PortraitWidth]];
    long now = [[[NSDate alloc] init] timeIntervalSince1970];
    if ((now - [createTime longLongValue]) < 15) {//防止连续刷新时，多次生成
        return;
    }
    
    [[NSUserDefaults standardUserDefaults] setObject:@(now) forKey:[NSString stringWithFormat:@"DMC_group_generate_portrait_time_%@_%d", groupId, PortraitWidth]];
    ...
    [[DMCCIMService sharedDMCIMService] getGroupMembers:groupId refresh:NO success:^(NSString *groupId, NSArray<DMCCGroupMember *> *members) {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            for (DMCCGroupMember *member in members) {
            ...
            for (int i = 0; i < row; i++) {
                ...
                for (int j = 0; j < c; j++) {
                    ...
                    __block DMCCUserInfo *user = nil;
                    [[DMCCIMService sharedDMCIMService] getUserInfo:userId refresh:NO success:^(DMCCUserInfo *userInfo) {
                        user = userInfo;
                    ...
                    if (user.portrait.length) {
                        image = [DMCCUtilities getUserImage:user.portrait];
                        if (!image) {
            ...
            NSString *mdt = [DMCCUtilities rf_EncryptMD5:fullPath];
            NSString *fileName = [NSString stringWithFormat:@"%@-%lld-%d-%@", groupId, now, PortraitWidth, mdt];

            NSString *path = [[DMCCUtilities getDocumentPathWithComponent:@"/group_portrait"] stringByAppendingPathComponent:fileName];
            ...
            [[NSUserDefaults standardUserDefaults] setObject:path forKey:[NSString stringWithFormat:@"DMC_group_generated_portrait_%@_%d", groupId, PortraitWidth]];
...
+ (NSString *)getGroupGridPortrait:(NSString *)groupId
               ...
               defaultUserPortrait:(UIImage *(^)(NSString *userId))defaultUserPortraitBlock {
    //Setp1 检查是否有此群组的记录
    NSString *path = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"DMC_group_generated_portrait_%@_%d", groupId, width]];
    
    if (!path.length) { //记录不存在，生成
        if (generateIfNotExist) {
            [DMCCUtilities generateNewGroupPortrait:groupId width:width defaultUserPortrait:defaultUserPortraitBlock];
        }
    } else { //记录存在
        //修正沙盒路径
        path = [DMCCUtilities getSendBoxFilePath:path];
        
        //检查文件是否存在
        NSFileManager* fileManager = [NSFileManager defaultManager];
        if ([fileManager fileExistsAtPath:path]) { //文件存在
            if (generateIfNotExist) {
                [[DMCCIMService sharedDMCIMService] getGroupInfo:groupId refresh:NO success:^(DMCCGroupInfo *groupInfo) {
                    //分析文件名，获取更新时间，hash值
                    ...
                    if (timestamp + 7 * 24 * 3600 < now || timestamp*1000 < groupInfo.updateTimestamp) {
                        [[DMCCIMService sharedDMCIMService] getGroupMembers:groupId refresh:NO success:^(NSString *groupId, NSArray<DMCCGroupMember *> *members) {
                            ...
                            for (int i = 0; i < MIN(members.count, 9); i++) {
                                ...
                                DMCCUserInfo *userInfo = [[DMCCIMService sharedDMCIMService] getUserInfo:userId refresh:NO];
...
+ (void)load {
    DMCUrlImageDict = [[NSMutableDictionary alloc] init];
    DMCImageLock = [[NSLock alloc] init];
}

// 是否数据库更新
+ (BOOL)isCheckVersionDBUpdate:(NSString *)currentVersion {
    NSString *key = @"DBdataVersion";
