 ...
 */
typedef NS_ENUM(NSInteger, DMCCPersistFlag) {
    DMCCPersistFlag_NOT_PERSIST = 0,
    DMCCPersistFlag_PERSIST = 1,
    DMCCPersistFlag_PERSIST_AND_COUNT = 3,
    DMCCPersistFlag_TRANSPARENT = 4,
};

/**
 普通消息的持久化内容
 */
@interface DMCCMessagePayload : NSObject
...
/**
 媒体消息的持久化内容
 */
@interface DMCCMediaMessagePayload : DMCCMessagePayload

/**
 媒体类型
 */
@property (nonatomic, assign)DMCCMediaType mediaType;
 ...
 */
@protocol DMCCMessageContent <NSObject>
...
/**
 消息内容，自定义消息可以继承此类
 */
@interface DMCCMessageContent : NSObject <DMCCMessageContent>
