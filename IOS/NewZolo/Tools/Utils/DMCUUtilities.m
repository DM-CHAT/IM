...
+ (NSString *)formatTimeDetailLabel:(int64_t)timestamp {
    ...
    if ([calendar isDateInToday:date]) {
        ...
        if(weeks == curWeeks) {
            return [NSString stringWithFormat:@"%@ %@", [DMCUUtilities formatWeek:weekDays], hourTimeStr];
        }
...
+ (CGRect) fixScreen:(CGRect) rc{
    //navibarHight = faceid ? 88.f : 64.f
    //statusbarHeight = faceid ? 44.f : 20.f
    //indicatorbarHeight = faceid ? 34.f : 0
    //tarbarHeight = 49.f
    float statusHeight = [[UIApplication sharedApplication] statusBarFrame].size.height;
    rc.origin.y += statusHeight;
//    if(statusHeight > 20){
//        rc.origin.y += 44;
//        rc.size.height -= 20;
//    }else{
