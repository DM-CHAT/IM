...
- (void)viewDidLoad {
    ...
    msgQueue = dispatch_queue_create("com.ospn.chatSigle", DISPATCH_QUEUE_CONCURRENT);
    _luck = [NSObject new];
...
- (void)getData {
    @synchronized (_luck) {
        self.conversations = [NSMutableArray arrayWithArray:[[DMCCIMService sharedDMCIMService] getConversationInfoTags:@[@(Single_Type)] lines:@[@(0)] tags:-1]];
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (DMCCUserInfo *)getUserInfo:(DMCCConversationInfo *)conv {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.userInfo = [self getUserInfo:info];
    if (info.isTop) {
