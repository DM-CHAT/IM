...
@interface ZoloRedPacketCell ()

@property (weak, nonatomic) IBOutlet UIImageView *redTagImg;
@property (weak, nonatomic) IBOutlet UILabel *redSubLab;
...
@implementation ZoloRedPacketCell
...
- (void)awakeFromNib {
    ...
    self.thumBg.userInteractionEnabled = YES;
    self.statusLab.text = LocalizedString(@"RedOrTransRed");
    ...
    [self.thumBg addGestureRecognizer:self.tapGes];
    [self.thumBg addGestureRecognizer:self.longGes];
...
+ (void)calculateCellHeight:(DMCCMessage *)message chatType:(BOOL)isGroup mulSelect:(BOOL)isMulSelect byMsg:(BOOL)isMySend { 
    message.msgCellHeight = 90 + 14;
    message.msgCellWidth = 210;
}
...
- (void)setMessage:(DMCCMessage *)message {
    ...
    if ([info.type isEqualToString:@"normal"]) { // 转账
        self.redTagImg.image = [UIImage imageNamed:@"tran_bg"];
        self.nameLab.hidden = YES;
        self.moneyLab.hidden = NO;
        self.redSubLab.hidden = NO;
        self.statusLab.hidden = YES;
        if ([message.fromUser isEqualToString:self.myInfo.userId]) {
            self.statusLab.hidden = YES;
            self.thumBg.image = [UIImage imageNamed:@"redBg_n"];
        } else {
            if (info.state == 0) {
                self.statusLab.text = LocalizedString(@"CollectWaitTitle");
                self.statusLab.hidden = NO;
                self.thumBg.image = [UIImage imageNamed:@"redBg_n"];
            } else {
                self.statusLab.text = LocalizedString(@"CollectRedTitle");
                self.statusLab.hidden = NO;
                self.thumBg.image = [UIImage imageNamed:@"redBg_s"];
            }
        }
    
        if (info.coinType.length > 0) {
            self.moneyLab.text = [NSString stringWithFormat:@"%@ %.6f", info.coinType,[info.price doubleValue] / 1000000];
        } else {
            self.moneyLab.text = [NSString stringWithFormat:@"USDT %.6f", [info.price doubleValue] / 1000000];
        }
        self.redSubLab.text = info.text;
    } else if ([info.type isEqualToString:@"loot"]) { // 手气
        self.redTagImg.image = [UIImage imageNamed:@"redBox"];
        self.nameLab.hidden = NO;
        self.moneyLab.hidden = YES;
        self.redSubLab.hidden = YES;
        if (info.state == 0) {
            self.statusLab.hidden = YES;
            self.thumBg.image = [UIImage imageNamed:@"redBg_n"];
        } else {
        ...
        if (info.state == 0) {
            self.statusLab.hidden = YES;
            self.thumBg.image = [UIImage imageNamed:@"group_redBg_n"];
        } else {
            self.statusLab.hidden = NO;
            self.thumBg.image = [UIImage imageNamed:@"group_redBg_s"];
        }
        self.nameLab.text = info.text;
    }
