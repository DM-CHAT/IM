...
@interface ZoloDappWebView ()  <WKNavigationDelegate, WKScriptMessageHandler>
...
@implementation ZoloDappWebView
...
- (UIButton *)closeBtn {
    if (!_closeBtn) {
        _closeBtn = [[UIButton alloc] initWithFrame:CGRectMake(16, 80, 50, 50)];
...
- (UIButton *)fullBtn {
    if (!_fullBtn) {
        _fullBtn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenwidth - 50 - 16, 80, 50, 50)];
        [_fullBtn setImage:[UIImage imageNamed:@"dapp_full"] forState:UIControlStateNormal];
...
- (void)setUpWebView {
    ...
    _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 64 + iPhoneX_topH, KScreenWidth, KScreenHeight / 2 - iPhoneX_bottomH - 64) configuration:configuration];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@", self.appInfo.url, self.appInfo.shareId];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if([command isEqualToString:@"GoBack"])
        ...
        [[MHCurrentVCTool getCurrentVC] presentViewController:nav animated:YES completion:nil];
        ...
        [[MHCurrentVCTool getCurrentVC] presentViewController:vc animated:YES completion:nil];
...
- (void)addGroup:(NSString *)groupId {
    // 判断群组是否已经存在
    if ([[DMCCIMService sharedDMCIMService] isFavGroup:groupId] || [self getConversation:groupId]) {
        ...
        [[MHCurrentVCTool getCurrentNav] pushViewController:[[ZoloSingleChatVC alloc] initWithConversationInfo:info] animated:YES];
    } else {
        ...
        [[MHCurrentVCTool getCurrentNav] pushViewController:vc animated:YES];
    }
}*/
//- (void)addGroup:(NSString *)groupId {
//    // 判断群组是否已经存在
...
- (void)addFriend:(NSString *)userId {
    ...
    if ([[DMCCIMService sharedDMCIMService] isMyFriend:userInfo.userId]) {
        [[MHCurrentVCTool getCurrentNav] pushViewController:[[ZoloContactDetailVC alloc] initWithUserInfo:userInfo] animated:YES];
    } else {
