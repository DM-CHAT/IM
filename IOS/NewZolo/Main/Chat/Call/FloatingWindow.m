...
- (void)animationDidStart:(CAAnimation *)theAnimation
{
    ...
    if (self.isExit) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ISAUDIOVIDEO" object:nil];
        if ([self.floatDelegate respondsToSelector:@selector(assistiveStart)]) {
            [self.floatDelegate assistiveStart];
        }
    }
}
...
//    UIBezierPath *maskFinalBP = [UIBezierPath bezierPathWithOvalInRect:CGRectInset(startFrame, -radius, -radius)];
    UIBezierPath *maskFinalBP = [UIBezierPath bezierPathWithRoundedRect:CGRectInset(startFrame, -radius, -radius) cornerRadius:10];
