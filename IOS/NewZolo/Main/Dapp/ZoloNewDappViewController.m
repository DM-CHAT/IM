...
@interface ZoloNewDappViewController () <WKNavigationDelegate, WKScriptMessageHandler>
...
@implementation ZoloNewDappViewController
...
- (void)setUpWebView {
    ...
    if (self.appInfo.urlParam.length > 0) {
        NSDictionary *dic = [OsnUtils json2Dics:self.appInfo.urlParam];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if([command isEqualToString:@"GoBack"])
        ...
        NSString *result = [OsnUtils dic2Json:args[@"Data"]];
        if (_payResultBlock) {
