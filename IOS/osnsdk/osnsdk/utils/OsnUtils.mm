...
+ (NSMutableDictionary*)json2Dics:(NSString*)data{
    return [OsnUtils json2Dic:[data dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (NSMutableDictionary*) wrapMessage:(NSString*) command from:(NSString*) from to:(NSString*) to data:(NSMutableDictionary*) data key:(NSString*) key{
    @try {
        ...
        json[@"content"] = [OsnUtils dic2Json:data];

        long timestamp = [OsnUtils getTimeStamp];
        NSString *calc = [from stringByAppendingFormat:@"%@%ld%@",to,timestamp,json[@"content"]];
        NSLog(@"calc: %@", calc);
...
+ (NSMutableDictionary*) takeMessage:(NSMutableDictionary*) json key:(NSString*) key osnID:(NSString*) osnID{
    @try {
        ...
        if(crypto == nil || [crypto isEqualToString:@"none"]){
            ...
            if([json[@"to"] isEqualToString:osnID])
