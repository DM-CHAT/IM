...
public class AppService implements AppServiceProvider {
    ...
    public void namePwdLogin(String account, String password, LoginCallback callback) {
        ...
        try {
            ...
            e.printStackTrace();
            callback.onUiFailure(-1, "network error...");
            return;
        }
    ...
    public void uploadLog(SimpleCallback<String> callback) {
        ...
        if (filePaths == null || filePaths.isEmpty()) {
            if (callback != null) {
                callback.onUiFailure(-1, "not log file");
            }
            return;
        }
        ...
        if (toUploadCount == 0) {
            if (callback != null) {
                callback.onUiFailure(-1, "all log file uploaded");
            }
        }
    }
    ...
    public static void validateConfig(Context context) {
        if (TextUtils.isEmpty(Config.IM_SERVER_HOST)
            ...
            Toast.makeText(context, "configure error, please check config, app will close...", Toast.LENGTH_LONG).show();
        ...
        for (String[] ice : Config.ICE_SERVERS) {
            if (!ice[0].startsWith("turn")) {
                Toast.makeText(context, "Turn config error，place check config，app will close...", Toast.LENGTH_LONG).show();
                new Handler().postDelayed(() -> {
                    throw new IllegalArgumentException("config error\n");
                }, 5 * 1000);
            }
