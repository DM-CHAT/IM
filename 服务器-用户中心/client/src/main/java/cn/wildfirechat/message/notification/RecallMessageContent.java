...
public class RecallMessageContent extends NotificationMessageContent {
    ...
    public String formatNotification(Message message, Context context) {
        String staticStr = context.getString(R.string.recall_one_message);
        if (staticStr.startsWith("wfc")) {
            staticStr = staticStr.substring(3);
        }
        String notification = "%s"+ staticStr;
        if (fromSelf) {
