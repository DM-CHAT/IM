...
public class CompositeMessageContent extends MessageContent {
    ...
    public String digest(Message message, Context context) {
        return context.getString(R.string.chat_record_digest1) + title;
    }

    public String compositeDigest(Activity activity) {
        ...
        for (int i = 0; i < messages.size() && i < 4; i++) {
            ...
            sb.append(userInfo.displayName + ": " + msg.content.digest(msg, activity));
            sb.append("\n");
        }
        return sb.toString();
    }
