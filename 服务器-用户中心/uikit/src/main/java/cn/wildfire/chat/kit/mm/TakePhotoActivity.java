...
//    public static final int BUTTON_STATE_BOTH = 0x103;              //两者都可以
    ...
    private void initView() {
        ...
        //设置小视频保存路径
        File file = new File(Config.getVideoSaveDir(this));
        if (!file.exists()) {
            file.mkdirs();
        }
        ...
        mJCameraView.setSaveVideoPath(Config.getVideoSaveDir(this));
        mJCameraView.setJCameraLisenter(new JCameraListener() {

            @Override
            public void captureSuccess(Bitmap bitmap) {
                //获取到拍照成功后返回的Bitmap
                String path = saveBitmap(bitmap, Config.getVideoSaveDir(TakePhotoActivity.this));
    ...
    public String saveBitmap(Bitmap bm, String dir) {
        String path = "";
        File f = new File(dir, "dm_" + SystemClock.currentThreadTimeMillis() + ".png");
        if (f.exists()) {
            f.delete();
        }
        try {
