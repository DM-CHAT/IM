...
public class MuteGroupMemberActivity extends BasePickGroupMemberActivity {
    ...
    protected void onGroupMemberChecked(List<UIUserInfo> checkedUserInfos) {
        ...
        if (checkedUserInfos == null || checkedUserInfos.isEmpty()) {
            menuItem.setTitle(R.string.confirm);
            menuItem.setEnabled(false);
        } else {
            menuItem.setTitle(getString(R.string.confirm)+"(" + checkedUserInfos.size() + ")");
    ...
    private void muteOrAllowGroupMembers() {
        ...
        MaterialDialog dialog = new MaterialDialog.Builder(this)
            .content(groupMuted ? R.string.adding_white_list : R.string.muting)
            .progress(true, 100)
            .cancelable(false)
            .build();
        dialog.show();
        Observer<OperateResult<Boolean>> observer =
            booleanOperateResult -> {
                ...
                if (booleanOperateResult.isSuccess()) {
                    ...
                    Toast.makeText(this, groupMuted ? getString(R.string.add_white_failed) : getString((R.string.mute_failed1)) + booleanOperateResult.getErrorCode(), Toast.LENGTH_SHORT).show();
                }
            };
        if (groupMuted) {
