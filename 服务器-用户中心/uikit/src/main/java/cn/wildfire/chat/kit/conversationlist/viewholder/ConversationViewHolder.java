...
public abstract class ConversationViewHolder extends RecyclerView.ViewHolder {
    ...
    protected ImageView nftImageView;
    ...
    public void onBind(ConversationInfo conversationInfo) {
        ...
        if(conversationInfo.conversation.target.startsWith("OSNU")){
        ...
        if (draft != null) {
            String draftString = draft.getContent() != null ? draft.getContent() : WfcUIKit.getString(R.string.draft);
            ...
            if (conversationInfo.unreadCount.unreadMentionAll > 0 || conversationInfo.unreadCount.unreadMention > 0) {
                promptTextView.setText(WfcUIKit.getString(R.string.user_a_me));
            ...
            if (conversationInfo.lastMessage != null && conversationInfo.lastMessage.content != null) {
                ...
                try {
                    if (conversationInfo.conversation.type == Conversation.ConversationType.Group
                        ...
                        content = senderDisplayName + ":" + lastMessage.digest(WfcUIKit.getActivity());
                    } else {
                        //content = lastMessage.digest();
                        content = lastMessage.digest(WfcUIKit.getActivity());
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public String contextMenuTitle(Context context, String tag) {
        ...
        switch (tag) {
            case ConversationContextMenuItemTags.TAG_CLEAR:
                title = WfcUIKit.getString(R.string.clean_conversation);
                break;
            case ConversationContextMenuItemTags.TAG_REMOVE:
                title = WfcUIKit.getString(R.string.delete_conversation);
                break;
            case ConversationContextMenuItemTags.TAG_TOP:
                title = WfcUIKit.getString(R.string.set_top);
                break;
            case ConversationContextMenuItemTags.TAG_CANCEL_TOP:
                title = WfcUIKit.getString(R.string.unset_top);
            default:
                break;
    ...
    public String contextConfirmPrompt(Context context, String tag) {
        ...
        switch (tag) {
            case ConversationContextMenuItemTags.TAG_CLEAR:
                title = WfcUIKit.getString(R.string.confirm_clean_conversation);
                break;
            case ConversationContextMenuItemTags.TAG_REMOVE:
                title = WfcUIKit.getString(R.string.confirm_delete_conversation);
                break;
        }
        return title;
    }
