...
public class PickContactActivity extends WfcBaseActivity {
    ...
    protected void updatePickStatus(List<UIUserInfo> userInfos) {
        if (userInfos == null || userInfos.isEmpty()) {
            confirmTv.setText(getString(R.string.confirm));
            menuItem.setEnabled(false);
        } else {
            confirmTv.setText(getString(R.string.confirm)+"(" + userInfos.size() + ")");
        ...
        if(sharePick){
            ...
            if(shareText != null){
                for(UIUserInfo uiUserInfo : newlyCheckedUserInfos){
                    ...
                    messageViewModel.sendShareMsg(userInfo.uid, shareText, getString(R.string.from_share), null, null, "身边大爱", "app://123456");
                }
            }
            else {
                try {
                    ...
                    ChatManager.Instance().uploadMedia(uri.getPath(), byteArrayOutputStream.toByteArray(), MessageContentMediaType.IMAGE.getValue(), new GeneralCallback2() {
                        ...
                        public void onSuccess(String result) {
                            for (UIUserInfo uiUserInfo : newlyCheckedUserInfos) {
                                UserInfo userInfo = uiUserInfo.getUserInfo();
                             //   messageViewModel.sendShareMsg(userInfo.uid, getString(R.string.test_share), getString(R.string.from_share), result, null, "test", "app://123456");
                                messageViewModel.sendShareMsg(userInfo.uid, "", "", result, null, "", "app://123456");
                            }
                            finish();
                        }
                        @Override
