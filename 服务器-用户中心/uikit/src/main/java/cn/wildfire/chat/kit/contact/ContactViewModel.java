...
public class ContactViewModel extends ViewModel implements OnFriendUpdateListener {
    ...
    public MutableLiveData<Boolean> rejectFriendRequest(String friendId) {

        MutableLiveData<Boolean> result = new MutableLiveData<>();
        ChatManager.Instance().handleFriendRequest(friendId, false, null, new GeneralCallback() {
            @Override
            public void onSuccess() {
                ChatManager.Instance().loadFriendRequestFromRemote();
                List<FriendRequest> inComingFriendRequests = ChatManager.Instance().getFriendRequest(true);
    ...
    public MutableLiveData<Boolean> acceptGroupRequest(String userId, String groupId) {

        MutableLiveData<Boolean> result = new MutableLiveData<>();
        ChatManager.Instance().handleGroupRequest(userId, groupId, true, new GeneralCallback() {
            @Override
            public void onSuccess() {
                ChatManager.Instance().loadFriendRequestFromRemote();
                List<FriendRequest> inComingFriendRequests = ChatManager.Instance().getFriendRequest(true);
                for (FriendRequest request : inComingFriendRequests) {
                    if (request.target.equals(groupId)) {
                        result.setValue(true);
                        return;
                    }
                }
    ...
    public MutableLiveData<Boolean> rejectGroupRequest(String userId, String gropuId) {

        MutableLiveData<Boolean> result = new MutableLiveData<>();
        ChatManager.Instance().handleGroupRequest(userId, gropuId, false, new GeneralCallback() {
            @Override
            public void onSuccess() {
                ChatManager.Instance().loadFriendRequestFromRemote();
                List<FriendRequest> inComingFriendRequests = ChatManager.Instance().getFriendRequest(true);
                for (FriendRequest request : inComingFriendRequests) {
                    if (request.target.equals(gropuId)) {
                        result.setValue(true);
                        return;
                    }
                }
