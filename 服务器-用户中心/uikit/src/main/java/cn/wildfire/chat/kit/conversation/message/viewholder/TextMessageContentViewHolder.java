...
public class TextMessageContentViewHolder extends NormalMessageContentViewHolder {
    ...
    public void onBind(UiMessage message) {
      ...
      //      refTextView.setText(quoteInfo.getUserDisplayName() + ": " + ((TextMessageContent) message.message.content).getContent());
        } else {
    ...
    public void onClick(View view) {
        ...
        //       WfcWebViewActivity.loadHtmlContent(fragment.getActivity(), WfcUIKit.getString(R.string.message_context), content);
        //      int messageUid = (int) quoteInfo.getMessageUid();
        //      fragment.test(messageUid);
    }

    @OnClick(R2.id.refTextView)
    public void onRefClick(View view) {
        Message message = ChatManager.Instance().getMessageByUid(quoteInfo.getMessageHash0());
        if(message == null){
            message = ChatManager.Instance().getMessageByHash(quoteInfo.getMessageHash());
        }
        if (message != null) {
            // TODO previewMessageActivity
            MessageContent messageContent = message.content;
            if (messageContent instanceof TextMessageContent) {
                WfcWebViewActivity.loadHtmlContent(fragment.getActivity(), WfcUIKit.getString(R.string.message_context), ((TextMessageContent) messageContent).getContent());
            } else {
    ...
    public String contextMenuTitle(Context context, String tag) {
        if (MessageContextMenuItemTags.TAG_CLIP.equals(tag)) {
            return WfcUIKit.getString(R.string.copy);
        }
