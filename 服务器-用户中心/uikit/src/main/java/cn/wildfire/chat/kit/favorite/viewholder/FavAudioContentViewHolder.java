...
public class FavAudioContentViewHolder extends FavContentViewHolder {
    ...
    void playAudio(){
        ...
        File file = messageViewModel.mediaMessageContentFile(message, fragment.getContext());
        if (file == null) {
            return;
        }
        if (file.exists()) {
            messageViewModel.playAudioMessage(uiMessage, fragment.getContext());
        } else {
            if (uiMessage.isDownloading) {
                return;
            }
            messageViewModel.downloadMedia(fragment.getActivity(),uiMessage, file);
        }
    }
}
