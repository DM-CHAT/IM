...
public class ImagePreviewActivity extends ImagePreviewBaseActivity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCbEdit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(ImagePreviewActivity.this, IMGEditActivity.class);
                try {
                    ImageItem item = mImageItems.get(mCurrentPosition);
        ...
        if (resultCode == Activity.RESULT_OK) {
            if(requestCode == 1234){
                ...
                if(mCbCheck.isChecked()){
                    ImageItem imageItem = mImageItems.get(mCurrentPosition);
                    store.addSelectedImageItem(mCurrentPosition, imageItem, false);
                }
                store.addSelectedImageItem(mCurrentPosition, item, false);
                mCbCheck.setChecked(true);
                mImageItems.add(item);
                mCurrentPosition = mImageItems.size()-1;
